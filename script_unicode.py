...
unicode_beg = 33    # Beggining Unicode Value
unicode_end = 700   # Ending Unicode Value
...
def initialize():
    global char_imap, unicode_beg, unicode_end, NORM
    for x in range(unicode_beg, unicode_end):
        im = imager(chr(x))
...
def generate_output(out_type, filename, image, mode):
    global types, mode_types, char_imap, TT_FONT_PATH
    w, h = image.size
    font = ImageFont.truetype(TT_FONT_PATH, 8)

    if out_type == 4:
        output = open('output/' + filename + '_uni_BW_%d-%d_art.txt' % (NORM, unicode_end), "w")
        ...
        if mode == 1:
            output.save('output/' + filename + '_uni_BW_%d-%d_art.' % (NORM, unicode_end) + ext)
        else:
            output.save('output/' + filename + '_uni_' + mode_types.get(mode) + '_%d-%d_art.' % (NORM, unicode_end) + ext)
...
def print_usage():
    ...
    print('''
    Usage:
        python script_unicode.py [image] [output_type] [mode]
    ...
    Example:
        python script_unicode.py images/wall.jpg 1 2
         - This loads the image wall.jpg from the "images" directory.
           ...
           And The Output generated named 'wall_RGB_uni_xxx-xxx_art.jpg' will be stored in "output" directory.
    ''')
